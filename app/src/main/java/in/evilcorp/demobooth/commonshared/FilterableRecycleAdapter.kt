...
import `in`.evilcorp.demobooth.commonshared.FilterableRecycleAdapter.BindableHolder
import `in`.evilcorp.demobooth.commonshared.FilterableRecycleAdapter.Item
...
abstract class FilterableRecycleAdapter<T : Item, H : BindableHolder<T>> : ListAdapter<T, H>(DiffCallback<T>()) {
    ...
    @Deprecated("Use 'setNewItems(list)' to maintain filterable", replaceWith = ReplaceWith("addNewItems(list)"))
